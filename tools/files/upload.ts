...
const s3 = new Minio.Client({
  endPoint: process.env.ASSET_STORE_END_POINT ?? "localhost",
  port: parseInt(process.env.ASSET_STORE_PORT ?? "9000"),
...
const getDetails = () => {
  prompt.start();
  prompt.message =
    "Enter project title, the object's name and the file's path:\n";
  prompt.delimiter = "";
  return prompt.get({
    properties: {
      projectTitle: {
...
const run = async () => {
  ...
  s3.fPutObject(
    projectTitle,
    objectName,
    filePath === null || filePath === undefined ? objectName : filePath,
  ).catch(console.error);
};
