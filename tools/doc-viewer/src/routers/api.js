...
const docs = components.apis ? glob.globSync(path.join(contentRoot, 'api', '*')).map(dir => ({
...
const apis = docs.map(({ header, files }) => ({
  ...
  files: files.map(({ label }) => ({
    href: `/api/view?urls.primaryName=${encodeURIComponent(`${header}/${label}`)}`,
    label
  }))
}));
...
router.use('/view', swaggerUi.serve, swaggerUi.setup(undefined, { swaggerUrls }));
