...
const coverage = components.coverage ? glob.globSync(path.join(contentRoot, 'coverage', 'tests', '*')).map(dir => ({
  header: dir.split('/').at(-1),
  files: glob.globSync(path.join(dir, '*')).map(x => ({
    href: `/${x.replace('coverage', 'coverage/static').split('/').slice(-5).join('/')}`,
    label: x.split('/').at(-1)
  }))
})) : [];
