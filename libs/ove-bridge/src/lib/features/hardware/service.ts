...
const removeDevice = async ({ deviceId }: BridgeServiceArgs<"removeDevice">) => {
...
export const deviceHandler = async <Key extends keyof DeviceService>(
  ...
  const response = await applyService<typeof k>(getServiceForProtocol(device.protocol), k, args as DeviceServiceArgs<Key>, device);
...
export const multiDeviceHandler = async <Key extends keyof DeviceService>(
  ...
  delete args["tag"];
  const result = await Promise.all(devices.map(device => applyService<Key>(getServiceForProtocol(device.protocol), k, args as DeviceServiceArgs<Key>, device)));
...
export const Service: BridgeService = {
  getDevice: async (args, callback) => wrapCallback(callback)({ response: await getDevice(args) }),
  getDevices: async (args, callback) => wrapCallback(callback)({ response: await getDevices(args) }),
  addDevice: async (args, callback) => wrapCallback(callback)({ response: await addDevice(args) }),
  removeDevice: async (args, callback) => wrapCallback(callback)({ response: await removeDevice(args) })
};