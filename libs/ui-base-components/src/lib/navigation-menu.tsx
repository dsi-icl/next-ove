...
>(({ className, children, ...props }, ref) => (
  <NavigationMenuPrimitive.Root
    ref={ref}
    className={cn(
      "relative z-10 flex flex-1 items-end justify-end bg-primary",
      className
    )}
    {...props}
  >
...
const navigationMenuTriggerStyle = cva(
  "inline-flex items-center justify-center rounded-md text-m " +
  "font-medium transition-colors focus:outline-none focus:bg-accent " +
  "text-white focus:text-accent-foreground disabled:opacity-50 " +
  "disabled:pointer-events-none bg-primary hover:bg-primary " +
  "hover:text-white data-[state=open]:bg-accent/50 " +
  "data-[active]:bg-accent/50 h-10 py-2 px-4 group w-max"
);
...
const NavigationMenuViewport = React.forwardRef<
  ...
  {className?: string | undefined}
>(({ className, ...props }, ref) => (
  <div className={cn("absolute top-full flex justify-center right-2")}>
...
>(({ className, ...props }, ref) => (
  ...
  >
    <div
      className="relative top-[60%] h-2 w-2 rotate-45
...
const NavigationMenuListItem = React.forwardRef<
  ...
  {className?: string | undefined, title?: string}
