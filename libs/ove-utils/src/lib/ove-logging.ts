...
export const Logger = (name?: string, logLevel?: number) => {
  ...
  const getLogLabel = (logLevel: LogLevel) => chalk.bgHex(logLevel.label.bgColor).hex(logLevel.label.color).bold;

  const buildLogMessage = (logLevel: LogLevel, ...args: string[]): string[] =>
    [(logLevel.name.length === 4 ? " " : "") +
    getLogLabel(logLevel)("[" + logLevel.name + "]"), dateFormat(new Date(), "dd/mm/yyyy, h:MM:ss.l tt"), "-",
      name_.padEnd(Constants.APP_LOG_ID_WIDTH), ":"].concat(Object.values(args));
  ...
  const log = (level: LogLevel, ...args: string[]): void => {
    ...
    switch (level.consoleLogger) {
      ...
      case 'log': console.log(...message)
        break;
      case 'trace': console.trace(...message)
        break;
    }
  }
