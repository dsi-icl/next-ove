...
const Configuration = ({setHasCalendar, openDialog}: ConfigurationProps) => {
  ...
  const updateCoreURL = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if ((e.nativeEvent as unknown as NativeEvent).submitter.name === "auto-mode") {
      openDialog();
      return;
    }
    ...
    await window.electron.updateEnv(coreURL, bridgeName, calendarURL === "" ? undefined : calendarURL).catch(console.error);
  };
  ...
  return <section className={styles.section}>
    ...
    <form method="post" onSubmit={e => updateCoreURL(e)} className={styles.form}>
      <label htmlFor="core-url">Core URL - {connected ? "connected" : "disconnected"}</label>
      ...
      <input id="calendar-url" type="text" name="calendar-url" defaultValue={displayCalendarURL} />
      <button id={styles["auto-mode"]} type="submit" name="auto-mode">Configure Auto Mode</button>
