...
const Controller = ({ config }: { config: TLaunchConfig }) => {
  ...
  return (
    <DialogContent
      className="m-0 block size-[90%] max-w-[unset] border-none p-0"
      hasClose={false}
    >
      <DialogHeader className="h-12 flex-row items-center space-y-0 rounded-t-lg bg-[#002147] p-2 font-semibold text-white">
        <DialogTitle className="ml-2">{project.data?.title ?? "-"}</DialogTitle>
        <div className="ml-auto mr-2 mt-0 flex h-full flex-row items-center">
          <Button
            ...
            onClick={() =>
              toast.promise(
                reloadBrowsers.mutateAsync({ bridgeId: config.observatory }),
                {
                  loading: "Reloading browsers...",
                  success: "Browsers reloaded successfully",
                  error: "Failed to reload browsers",
                },
              )
            }
          >
