...
export const useInitSections = () => {
  ...
  const {
    status,
    data
  } = api.projects.getSectionsForProject.useQuery({ projectId: projectId ?? "ERROR" }, { enabled: projectId !== null });
...
export const useUpdateSection = () => {
  ...
  return useCallback((section: Omit<Section, "id">) => {
    setSections(cur => {
...
export const useDragSection = () => {
  ...
  return useCallback((id: string, x: number, y: number) => {
...
export const useUpdateStateForSections = () => {
  ...
  return useCallback((state: string, name: string) => {
...
export const useSections = () => {
  ...
  const addToState = useCallback((id: string, state: string) => {
    setSections(cur =>
      cur.map(section =>
  ...
  const removeFromState = useCallback((id: string, state: string) => {
    if (id === selected) {
      setSelected(null);
    }
    const filter = (section: Section) =>
  ...
  const generateSection = useCallback((state: string, projectId: string) => {
  ...
  const getSectionsInState = useCallback((state: string) => sections.filter(({states}) => states.includes(state)), [sections]);
  const getSectionsToImport = useCallback((from: string, to: string) => sections.filter(({states}) => states.includes(from) && !states.includes(to)), [sections]);
