...
const StateTabs = () => {
  ...
  const onSubmit = useCallback(({ name }: TitleForm) => {
  ...
  return <nav
    className="overflow-x-scroll border-b border-solid border-[#dadedf] h-8">
    <ul className="items-start flex overflow-x-scroll p-1 gap-1">
      <DefaultTab key="default-tab"
                  isSelected={env.CONSTANTS.DEFAULT_STATE === selected} />
      {states.map(state =>
        state !== env.CONSTANTS.DEFAULT_STATE ? <Tab
          ...
          isEditing={isEditing} onSubmit={onSubmit} /> : null)}
      <AddTab key="add-tab" />
    </ul>
  </nav>;
};
...
const Tab = ({
  ...
  return <li style={{ width: `calc(${width} + 2rem)` }}
             className="grow basis-0 bg-[#dadedf] rounded text-center relative">
    <TabContent isBeingEdited={isBeingEdited} state={state}
                isSelected={isSelected} stopEditing={stopEditingTab}
                startEditing={editTab}>
      {isBeingEdited ?
        <EditTab onSubmit={onSubmit} /> :
        <p className="whitespace-nowrap mr-6" style={{ width }}>{title}</p>}
    </TabContent>
    <RemoveTab state={state} isEditing={isEditing} />
  </li>;
};

const DefaultTab = ({ isSelected }: {
  isSelected: boolean,
}) => {
  const setState = useStateStore(state => state.setSelectedState);

  return <li
    className={cn("bg-[#dadedf] rounded text-center relative min-w-[calc(1ch+2rem)] max-w-[calc(1ch+2rem)]", isSelected ? "font-bold" : undefined)}>
    <button onClick={() => setState(env.CONSTANTS.DEFAULT_STATE)}
            className="w-full">*
    </button>
  </li>;
};
...
const TabContent = ({
  ...
  return <button
    ...
    onClick={isBeingEdited ? undefined : () => setState(state)} onBlur={onBlur}
...
const EditTab = ({ onSubmit }: EditTabProps) => {
  ...
  return <form onSubmit={handleSubmit(onSubmit)}>
    <input className="w-full" {...register("name", { required: true })}
           autoFocus={true} />
    <input type="submit" className="w-full hidden" />
  </form>;
};
...
const AddTab = ({}) => {
  ...
  const onClick = useCallback(() => {
    addState();
    setTimeout(() => ref.current?.scrollIntoView({ behavior: "smooth" }), env.CONSTANTS.STATE_TAB_TRANSITION);
  }, [addState]);

  return <li className="flex-initial bg-[#dadedf] rounded text-center relative">
    <button className="px-2" ref={ref} onClick={onClick}>+</button>
  </li>;
};
