...
function drawObservatory(sections: Section[], container: {
  width: number,
  height: number
}, dragSection: (id: string, x: number, y: number) => void, select: (id: string) => void, selected: string | null, bounds: Bounds | null, cells: Geometry[], svg_: RefObject<SVGSVGElement | null>) {
  const x = d3.scaleLinear()
    .range([0, container.width]).domain([0, assert(bounds).width]);
  const inverseX = d3.scaleLinear()
    .range([0, assert(bounds).width]).domain([0, container.width]);
  const y = d3.scaleLinear()
    .range([0, container.height]).domain([0, assert(bounds).height]);
  const inverseY = d3.scaleLinear()
    .range([0, assert(bounds).height]).domain([0, container.height]);
    ...
    for (const cell of assert(cells)) {
      if (Math.abs(cell.x - x) <
        ((assert(bounds).width / assert(bounds).columns) *
          env.CONSTANTS.DRAG_SENSITIVITY.X)) return cell.x;
      if (Math.abs((cell.x + cell.width) - (x + w)) <
        ((assert(bounds).width / assert(bounds).columns) * env.CONSTANTS.DRAG_SENSITIVITY.X)) {
    ...
    for (const cell of assert(cells)) {
      if (Math.abs(cell.y - y) <
        ((assert(bounds).height / assert(bounds).rows) *
          env.CONSTANTS.DRAG_SENSITIVITY.Y)) return cell.y;
      if (Math.abs((cell.y + cell.height) - (y + h)) <
        ((assert(bounds).height / assert(
          bounds).rows) * env.CONSTANTS.DRAG_SENSITIVITY.Y)) {
  ...
  function dragging(this: Element, event: {
    ...
    const nx = Math.max(0, Math.min(x(assert(bounds).width) -
      parseFloat(section.attr("width")), x(event.subject.x) +
      (event.x - event.subject.x)));
    const ny = Math.max(0, Math.min(y(assert(bounds).height) -
...
const Canvas = () => {
  ...
  const sections = useMemo(() => getSections(selectedState).map(s => ({
    ...s,
  ...
  if (bounds !== null && cells !== null) {
    drawObservatory(sections, canvas, dragSection, setSelectedSection, selectedSection, bounds, cells, svg_);
  } else {
    d3.select(svg_.current).selectAll("*").remove();
  }

  return <ResizeContainer canvas={canvas}>
    <svg ref={svg_} width={canvas.width}
