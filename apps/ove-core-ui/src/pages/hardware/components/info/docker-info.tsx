...
const DockerInfo = ({ info }: {
    ...
    <Table>
      ...
      <TableBody>
        ...
        <TableRow>
          ...
          <TableCell>{new Date(image?.created ?? 0).toISOString()}</TableCell>
        </TableRow>
        <TableRow>
  ...
  {info.containers?.map((container, i) => <div key={container.id}>
    ...
    <Table>
      ...
      <TableBody>
        ...
        <TableRow>
          <TableCell>created</TableCell>
          <TableCell>{new Date(container?.created).toISOString()}</TableCell>
        </TableRow>
        <TableRow>
          <TableCell>started</TableCell>
          <TableCell>{new Date(container?.started).toISOString()}</TableCell>
        </TableRow>
        <TableRow>
          <TableCell>finished</TableCell>
          <TableCell>{new Date(container?.finished).toISOString()}</TableCell>
        </TableRow>
        <TableRow>
  ...
  {info.containers?.flatMap(container => container.mounts.map(mount => ({ id: container.id, ...mount }))).map((mount, i) =>
    <div key={mount.id}>
