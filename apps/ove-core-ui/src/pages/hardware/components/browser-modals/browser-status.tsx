...
const BrowserStatus = () => {
  ...
  const getMaxLen = (browsers_: typeof browsers) => Array.isArray(browsers_) ? browsers_.length : null;

  return <PaginatedDialog data={browsers} maxLen={getMaxLen(browsers)}>
    <div className={styles.status}>
      <h4>Browsers
        - {Array.isArray(browsers) ? browsers[paginationIdx].deviceId : assert(deviceAction.deviceId)}</h4>
      <table>
        <thead>
        <tr>
          <th>Browser ID</th>
        ...
        <tbody>
        {Array.from((Array.isArray(browsers) ? browsers[paginationIdx].response : browsers).entries()).map(([k, v]) =>
          <tr key={k}>
            <td>{k}</td>
            <td>{v.displayId}</td>
            <td>{v.url}</td>
