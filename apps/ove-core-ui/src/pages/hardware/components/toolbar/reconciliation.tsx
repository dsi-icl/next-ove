...
const useReconciliation = (bridgeId: string) => {
  ...
  const start = useCallback(async () => {
    await startReconciliation.mutateAsync({ bridgeId })
      .catch(logger.error);
    apiUtils.bridge.getReconciliation.invalidate({ bridgeId }).catch(logger.error);
  ...
  const status = useMemo(() => {
    if (reconciliationStatus.status !== "success" || isError(reconciliationStatus.data.response)) return null;
