...
const AutoSchedule = ({ bridgeId }: { bridgeId: string }) => {
  ...
  const currentSchedule = useMemo(() => {
    if (
      getAutoSchedule.status !== "success" ||
      getAutoSchedule.data === undefined
    )
      return undefined;
    return {
      start: getAutoSchedule.data.wake ?? "",
      end: getAutoSchedule.data.sleep ?? "",
      days: (getAutoSchedule.data.schedule ?? defaultDaySelection).map(
        (day) => ({ value: day }),
      ),
    };
  }, [getAutoSchedule.status, getAutoSchedule.data]);
    ...
    ({ start, end, days }: AutoModeForm) => {
      toast.promise(
        ...
        }),
        {
          ...
          success: "Successfully updated auto schedule",
          error: "Unable to update auto schedule",
        },
      );
    },
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
                ...
                <FormItem>
                  ...
                  <InputGroup>
                    ...
                    <InputGroupAddon align="inline-end">
                      <InputGroupButton
                        onClick={() => form.setValue("start", undefined)}
                      >
                        <X />
                      </InputGroupButton>
                    </InputGroupAddon>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="mt-4">
                  ...
                  <InputGroup>
                    ...
                    <InputGroupAddon align="inline-end">
                      <InputGroupButton
                        onClick={() => form.setValue("end", undefined)}
                      >
                        <X />
                      </InputGroupButton>
                    </InputGroupAddon>
              ...
              name="end"
            />
            <h2 className="mt-4 text-sm font-medium">Days of Operation</h2>
