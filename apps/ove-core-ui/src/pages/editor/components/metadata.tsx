...
const MetadataFormSchema = ProjectMetadataSchema.omit({
  id: true,
  publications: true,
  tags: true,
  thumbnail: true,
...
const Metadata = () => {
  ...
  const { collaborators, users, inviteCollaborator, removeCollaborator } =
    ...
    <DialogContent>
      ...
      <Form {...form}>
        <form
          ...
          className="flex max-h-[65vh] flex-col overflow-y-scroll px-1"
        >
          <FormField
            name="title"
          ...
          />
          <Label htmlFor="file-select" className="mt-4 font-semibold">
            Thumbnail
          </Label>
          <div className="flex items-center">
            ...
            <Button
              ...
              type="button"
              className="ml-1 mt-2 h-[unset] rounded-[50%] p-2"
            >
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="mt-4 flex flex-row items-start space-y-0">
                ...
                <FormControl className="mt-0 flex flex-row place-items-center">
                  <Checkbox
                    {...field}
                    checked={field.value}
                    className="ml-auto"
          ...
          />
          <div ref={tagRef} className="flex size-full flex-col">
            <FormField
              name="tag"
              control={form.control}
              render={({ field }) => (
                <FormItem className="mt-4 flex w-full flex-col">
                  ...
                  <Popover open={tagOpen} onOpenChange={setTagOpen}>
                    ...
                    <PopoverContent
                      container={tagRef.current ?? undefined}
                      className="w-[calc(90%-4px)] p-0"
                    >
                      <Command>
                        <FormControl>
                          <CommandInput
                            onValueChange={(v) => form.setValue("tag", v)}
                            onKeyUp={({ key }) => {
                              if (key !== "Enter") return;
            ...
            {selectedTags.length > 0 ? (
              <ul className="mt-4 flex w-full flex-wrap gap-1 p-1">
                {selectedTags.map((tag) => (
                  <li
                    key={tag}
                    className="flex size-fit items-center rounded-3xl px-2 py-1 text-white"
                    style={{
                      backgroundColor:
                        actionColors[sumText(tag) % actionColors.length],
                    }}
                  >
                    {tag}
                    <Button
                      type="button"
                      variant="ghost"
                      className="m-0 ml-1 size-fit bg-transparent p-0"
                      onClick={() =>
                        setSelectedTags((cur) => cur.filter((t) => t !== tag))
                      }
                    >
                      <X />
                    </Button>
          ...
          <div ref={publicationRef} className="flex size-full flex-col">
            <FormField
              name="publication"
              control={form.control}
              render={({ field }) => (
                <FormItem className="mt-4 flex w-full flex-col">
                  <FormLabel className="font-semibold">Publications</FormLabel>
                  <Popover
                    ...
                    >
                      <Command>
                        <FormControl>
                          <CommandInput
                            onValueChange={(v) =>
                              form.setValue("publication", v)
                            }
                            onKeyUp={({ key }) => {
                              if (key !== "Enter") return;
                              if (publications.includes(field.value ?? ""))
                                return;
                          ...
                          <CommandGroup>
                            {publications.map((publication) => (
                              <CommandItem
                                ...
                                onSelect={() => {
                                  if (
                                    selectedPublications.includes(publication)
                                  ) {
                                    setSelectedPublications((prev) =>
                ...
                {selectedPublications.map((publication) => (
                  <li
                    ...
                    style={{
                      backgroundColor:
                        actionColors[
                          sumText(publication) % actionColors.length
                        ],
                    }}
                  >
                    {publication}
                    <Button
                      variant="ghost"
                      type="button"
                      className="m-0 ml-1 size-fit p-0"
                      onClick={() =>
                        setSelectedPublications((cur) =>
                          cur.filter((p) => p !== publication),
                        )
                      }
                    >
                      <X />
          ...
          <div className="mt-4 w-full" ref={collaboratorRef}>
              ...
              {collaborators.map((collaborator) => (
                ...
                >
                  <div className="flex items-center space-x-4">
                    <Avatar>
                      <AvatarImage
                        src={collaborator.icon ?? undefined}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="mt-4">
                <FormLabel className="font-semibold">Presenter Notes</FormLabel>
                <FormControl>
                  <Textarea {...field} />
                </FormControl>
              </FormItem>
