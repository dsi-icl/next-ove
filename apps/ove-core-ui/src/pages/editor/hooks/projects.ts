...
export const useProjectId = () => {
  ...
  return useMemo(() => query.get("project") ?? null, [query]);
};
...
export const useInitProject = (user: Omit<User, "password"> | null) => {
  ...
  const getProject = api.projects.getProject.useQuery({ projectId: projectId ?? "ERROR" }, { enabled: projectId !== null });
  const setProject = useProjectStore(state => state.setProject);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    if (getProject.status !== "success" || isError(getProject.data) || getProject.data === null) return;
    setProject({
      ...getProject.data,
...
export const useProjectStore = create<ProjectStore>(set => ({
  ...
  setProject: arg => set(state => ({ project: typeof arg === "function" ? arg(state.project) : arg }))
}));