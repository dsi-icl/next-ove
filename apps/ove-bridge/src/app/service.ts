...
const deviceWrapper = async <T>(
  callback: (device: Device) => Promise<Optional<DeviceResponse<T>>>,
  deviceId: string
) => {
  ...
  if (result === undefined) {
    return OVEUtils.raise("Command not available on device");
  }

  return result;
};

const multiDeviceWrapper = async <T>(
  callback: (device: Device) => Promise<Optional<DeviceResponse<T>>>,
  tag?: string
) => {
  ...
  if (isAll(z.undefined(), results)) {
    return OVEUtils.raise("Command not available on devices");
  }
...
export const getBrowserStatus = async (deviceId: string, browserId: number) => {
  return deviceWrapper((device: Device) =>
    _getBrowserStatus(device, browserId), deviceId);
};

export const getBrowserStatusAll = async (browserId: number, tag?: string) => {
  return multiDeviceWrapper((device: Device) =>
    _getBrowserStatus(device, browserId), tag);
};
...
export const screenshot = async (
  ...
  return deviceWrapper((device: Device) =>
    _screenshot(device, method, screens), deviceId);
};

export const screenshotAll = async (
  method: ScreenshotMethod,
  screens: ID[],
  tag?: string
) => {
  return multiDeviceWrapper((device: Device) =>
    _screenshot(device, method, screens), tag);
};
...
export const openBrowser = async (deviceId: string, displayId?: ID) => {
  return deviceWrapper((device: Device) =>
    _openBrowser(device, displayId), deviceId);
};

export const openBrowserAll = async (displayId?: ID, tag?: string) => {
  return multiDeviceWrapper((device: Device) =>
    _openBrowser(device, displayId), tag);
};
...
export const setVolume = async (deviceId: string, volume: number) => {
  return deviceWrapper((device: Device) =>
    _setVolume(device, volume), deviceId);
};

export const setVolumeAll = async (
  volume: number,
  tag?: string
) => {
  return multiDeviceWrapper((device: Device) =>
    _setVolume(device, volume), tag);
};

const _setSource = async (
  device: Device,
  source: z.infer<typeof SourceSchemas>,
  channel?: number
) => {
  const service = getServiceForProtocol(device.protocol);
...
export const setSource = async (
  ...
  return deviceWrapper((device: Device) =>
    _setSource(device, source, channel), deviceId);
};

export const setSourceAll = async (
  source: z.infer<typeof SourceSchemas>,
  channel?: number,
  tag?: string
) => {
  return multiDeviceWrapper((device: Device) =>
    _setSource(device, source, channel), tag);
};
...
export const closeBrowser = async (deviceId: string, browserId: number) => {
  return deviceWrapper((device: Device) =>
    _closeBrowser(device, browserId), deviceId);
};

export const closeBrowserAll = async (browserId: number, tag?: string) => {
  return multiDeviceWrapper((device: Device) =>
    _closeBrowser(device, browserId), tag);
};
