...
import {init} from "./hardware/controller";
import { type DesktopCapturerSource, type NativeImage } from "electron";
...
describe("OVE Client Router", () => {
  ...
  it("can take a screenshot of one screen of the device", async () => {
    ...
    const closeWindow = jest.fn((idx) => {
      console.log(`Closing window ${idx}`);
      return true;
    });
